"""
app.py — Mastercard DMS ISO 8583 Decoder, web edition (Flask)

Standalone sibling of the VisaNet decoder site - same architecture, own
Render service, own free-tries counter, own license-key file. All actual
ISO 8583 decoding is delegated to mastercard_iso8583_parser.py
(unmodified) through mastercard_adapter.py - this file only adds:
routing, the free-tries counter, license-key unlocking, and file export
(TXT / Excel / Field-Details download).
...
2. keys.json is a plain file on local disk. On most free hosting tiers
   ...
   beyond a quick MVP, move key storage to a real database.
3. Don't log or persist the raw pasted hex beyond the request lifecycle
   ...
   in the page footer.
4. Exported TXT/XLSX/Details reports are generated in-memory per request
   and streamed straight to the browser as a download - nothing is
   written to permanent disk storage on the server.

DEPLOYMENT NOTE: this is a fully separate app from the Visa site. Deploy
it as its own Render web service, with its own SECRET_KEY environment
variable, pointed at this folder (it has its own requirements.txt,
keys.json will be created fresh here, etc).
"""
...
from mastercard_adapter import (
    decode_mastercard_message,
    _prepare_hex_input,
    _extract_rrn,
    _extract_mti,
    HAS_OPENPYXL,
...
def parse_message():
    ...
    result, warnings, compact = decode_mastercard_message(raw)
...
def export_report(fmt):
    ...
    TXT, XLSX, or field-Details report back as a file download
    (mc_parser.write_txt_report / write_xlsx_report / write_detail_report,
    unmodified).
    ...
    if fmt == "details" and not HAS_FIELD_DETAILS:
        return jsonify({"error": "Field details export isn't available on this server (mastercard_field_details.py not found)."}), 400
    ...
    name = mc_parser.get_report_name(rows, mti)
    ...
    try:
        ...
        if fmt == "txt":
            mc_parser.write_txt_report(tmp_path, raw_hex_for_report, compact, rows)
            mimetype = "text/plain; charset=utf-8"
            download_name = f"{name}.txt"
        elif fmt == "xlsx":
            mc_parser.write_xlsx_report(tmp_path, raw_hex_for_report, compact, rows)
            mimetype = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            download_name = f"{name}.xlsx"
        else:  # details
            mc_parser.write_detail_report(tmp_path, rows)
            mimetype = "text/plain; charset=utf-8"
